...
dias.forEach(dia => {
calendario.appendChild(createCell(dia, "calendario-header"));
});

// se cran las fials de horas
horas.forEach(hora => {
dias.forEach((dia) => {
const horario = `${dia} ${hora}`;

// le doy la clase para el estilo
const celda = createCell(hora, horariosNoPermitidos.includes(horario) ? "calendario-dia no-permitido" : "calendario-dia");

// Deshabilitar interacción si el horario no está permitido
if (!horariosNoPermitidos.includes(horario)) {
  celda.onclick = () => {
  ...
  celda.title = "Este horario no está disponible";
}
...
function agregarBotonRegresarPregunta4() {
...
// Crear celda con la clase apropiada
const celda = createCell(hora, horariosNoPermitidos.includes(horario) ? "calendario-dia no-permitido" : "calendario-dia");
...
// Verificar si se seleccionaron suficientes horas
console.log(horas_a_realizar)
if (horariosSeleccionados.length < horas_a_realizar) {
alert(`Por favor, seleccione un mínimo de ${horas_a_realizar} horas.`);
return;
}

// Verificar si hay suficientes bloques de 3 horas para las preferencias
...
// Buscar bloques de 3 horas consecutivas por día
let bloquesContinuos = 0;
Object.values(horariosPorDia).forEach(horasDelDia => {
// Ordenar las horas del día para verificar continuidad
horasDelDia.sort((a, b) => ordenarHoras(a, b));

for (let i = 0; i < horasDelDia.length - 2; i++) {
if (
  verificarContinuidad(horasDelDia[i], horasDelDia[i + 1]) &&
  verificarContinuidad(horasDelDia[i + 1], horasDelDia[i + 2])
) {
  bloquesContinuos++;
  i += 2; // Saltar al siguiente conjunto después de contar este bloque
}
}
});

const total3Juntas = elecciones.filter(curso => curso.preferencia === "3-juntas").length;

if (bloquesContinuos < total3Juntas) {
alert(
`Faltan bloques de 3 horas continuas. Necesitas al menos ${total3Juntas} bloques de 3 horas continuas, y actualmente tienes ${bloquesContinuos}.`
);
return;
}
...
console.log(nombre);
mostrarCarga();
// Llamada al servidor para guardar los horarios
google.script.run
.withSuccessHandler(() => {
horariosSeleccionados2=[...horariosSeleccionados]
horariosSeleccionados = []; // Limpia la selección
document.querySelectorAll(".seleccionado").forEach(celda => {
  celda.classList.remove("seleccionado");
});

mostrarConfirmacion("Horarios guardados exitosamente.");
mostrarPreguntaExamen();
})
.withFailureHandler(errorCarga) // Manejo de errores
.todo_bien()
}
...
mostrarCalendario2(); // Avanzar a pregunta3
})
.withFailureHandler(errorCarga)
.todo_bien()
}
